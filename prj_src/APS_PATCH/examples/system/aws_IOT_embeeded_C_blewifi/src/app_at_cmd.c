...
typedef struct
{
    ...
    uint32_t u32IdFixSize;      // the fix length of file ID in FIM.
...
int write_fim_handle(uint32_t u32Type, uint8_t *u8aData, uint32_t u32DataLen, void *pParam)
{
    ...
    if(ptParam->fIgnoreRestString)
    {
			  msg_print_uart1("reset string...\r\n");
        goto done;
    }
    
    for(i = 0 ; i < u32DataLen ; i++)
    {
        ptParam->ResultBuf[i] = u8aData[i];
                  
        /* Result index add one */
        ptParam->u16Resultindex++;
    }
		msg_print_uart1("write...\r\n");
		//strncpy((char *)(ptParam->ResultBuf[0] + ptParam->u16Resultindex), (char *)u8aData, u32DataLen);
		//ptParam->u16Resultindex += u32DataLen;
    ...
    if(ptParam->u16Resultindex == ptParam->u16DataTotalLen)
    {
       	...
       	if(MW_FIM_OK == MwFim_FileWrite(ptParam->u32Id, ptParam->u16Index, ptParam->u32IdFixSize, ptParam->ResultBuf))
        {
            msg_print_uart1("OK\r\n");
        }
        else
        {
            ptParam->fIgnoreRestString = 1;
					  msg_print_uart1("MwFim_FileWrite error\r\n");
        }
    }
    else
    {
			  msg_print_uart1("No MwFim_FileWrite error\r\n");
        goto done;
    }
...
int app_at_cmd_sys_write_fim(char *buf, int len, int mode)
{
    ...
    switch(mode)
    {
        case AT_CMD_MODE_SET:
        {
					  if(tAtFimParam->u32Id == MW_FIM_IDX_GP13_PROJECT_AWS_PRIVATE_KEYS)
						{
							    tAtFimParam->u32IdFixSize = PRIVATE_KEY_SIZE;
						}
						else if (tAtFimParam->u32Id == MW_FIM_IDX_GP13_PROJECT_AWS_CERT_PEM)
						{
							    tAtFimParam->u32IdFixSize = CERT_PEM_SIZE;
						}
						else
						{
							goto done;
            ...
            /* Memory allocate a memory block for pointer */
            tAtFimParam->ResultBuf = (uint8_t *)malloc(tAtFimParam->u32IdFixSize);
            if(tAtFimParam->ResultBuf == NULL)
                goto done;
						memset(tAtFimParam->ResultBuf, 0, tAtFimParam->u32IdFixSize);
						tAtFimParam->TotalSize = tAtFimParam->u16DataTotalLen;
						
						msg_print_uart1("fetch data\r\n");
...
int app_at_cmd_sys_aws_dev(char *buf, int len, int mode)
{
    int iRet = 0;
    int argc = 0;
    char *argv[AT_MAX_CMD_ARGS] = {0};

    T_MwFim_GP13_AWS_DEVICE_INFO tDev = {0};
		
    switch (mode)
    {
        case AT_CMD_MODE_READ:
        {
            if(MwFim_FileRead(MW_FIM_IDX_GP13_PROJECT_AWS_DEVICE_INFO, 0, MW_FIM_GP13_AWS_DEVICE_INFO_SIZE, (uint8_t*)&tDev) != MW_FIM_OK)
            {
                goto done;
            }

            msg_print_uart1("aws host:\t%s\r\n", tDev.host);
            msg_print_uart1("aws client id:\t%s\r\n", tDev.client_id);
            msg_print_uart1("aws thing name:\t%s\r\n", tDev.thing_name);
            break;
        }
    
        case AT_CMD_MODE_SET:
        {
            uint32_t u32HostSize = 0;
            uint32_t u32ClientIDSize = 0;
            uint32_t u32ThingNameSize = 0;
            ...
            u32HostSize = sizeof(tDev.host);
            
            if(strlen(argv[1]) >= u32HostSize)
            {
                msg_print_uart1("Invalid host Length\r\n");
                goto done;
            }

            u32ClientIDSize = sizeof(tDev.client_id);

            if(strlen(argv[2]) >= u32ClientIDSize)
            {
                msg_print_uart1("Invalid client id Length\r\n");
                goto done;
            }

            u32ThingNameSize = sizeof(tDev.thing_name);

            if(strlen(argv[3]) >= u32ThingNameSize)
            {
                msg_print_uart1("Invalid thing name Length\r\n");
                goto done;
            }

            strncpy(tDev.host, argv[1], strlen(argv[1]));
						tDev.host[strlen(argv[1])] = '\0';
            strncpy(tDev.client_id, argv[2], strlen(argv[2]));
						tDev.client_id[strlen(argv[2])] = '\0';
            strncpy(tDev.thing_name, argv[3], strlen(argv[3]));
						tDev.thing_name[strlen(argv[3])] = '\0';

            if(MwFim_FileWrite(MW_FIM_IDX_GP13_PROJECT_AWS_DEVICE_INFO, 0, MW_FIM_GP13_AWS_DEVICE_INFO_SIZE, (uint8_t*)&tDev) != MW_FIM_OK)
            {
                goto done;
            }
...
int app_at_cmd_sys_aws_key(char *buf, int len, int mode)
{
    int iRet = 0;
    int argc = 0;
	  int i = 0;
    char *argv[AT_MAX_CMD_ARGS] = {0};
    uint32_t u32P11KeySize = 0;

    T_MwFim_GP13_AWS_PRIVATE_KEY tKey = {0};
    u32P11KeySize = sizeof(tKey.PrivateKey);

    switch (mode)
    {
        case AT_CMD_MODE_READ:
        {
            if(MwFim_FileRead(MW_FIM_IDX_GP13_PROJECT_AWS_PRIVATE_KEYS, 0, MW_FIM_GP13_AWS_PRIVATE_KEY_SIZE, (uint8_t*)&tKey) != MW_FIM_OK)
            {
                goto done;
            }

            msg_print_uart1("aws private key:\r\n", tKey.PrivateKey);
						for(i=0; i<u32P11KeySize; i++)
						    msg_print_uart1("%c", tKey.PrivateKey[i]);
        ...
        case AT_CMD_MODE_SET:
        {
            ...
            strncpy(tKey.PrivateKey, argv[1], key_len);
						tKey.PrivateKey[key_len] = '\0';

            if(MwFim_FileWrite(MW_FIM_IDX_GP13_PROJECT_AWS_PRIVATE_KEYS, 0, MW_FIM_GP13_AWS_PRIVATE_KEY_SIZE, (uint8_t*)&tKey) != MW_FIM_OK)
            {
                goto done;
            }
...
int app_at_cmd_sys_aws_cert(char *buf, int len, int mode)
{
    int iRet = 0;
    int argc = 0;
	  int i = 0;
    char *argv[AT_MAX_CMD_ARGS] = {0};
    uint32_t u32P11CertSize = 0;

    T_MwFim_GP13_AWS_CERT_PEM tCert = {0};
    u32P11CertSize = sizeof(tCert.CertPEM);

    switch (mode)
    {
        case AT_CMD_MODE_READ:
        {
            if(MwFim_FileRead(MW_FIM_IDX_GP13_PROJECT_AWS_CERT_PEM, 0, MW_FIM_GP13_AWS_CERT_PEM_SIZE, (uint8_t*)&tCert) != MW_FIM_OK)
            {
                goto done;
            }

            msg_print_uart1("aws certificate:\r\n", tCert.CertPEM);
						for(i=0; i<u32P11CertSize; i++)
						    msg_print_uart1("%c", tCert.CertPEM[i]);
        ...
        case AT_CMD_MODE_SET:
        {
            ...
            cert_len = strlen(argv[1]);
						
            if(cert_len >= u32P11CertSize)
            {
                msg_print_uart1("Invalid certificate Length\r\n");
                goto done;
            }


            strncpy(tCert.CertPEM, argv[1], cert_len);
						tCert.CertPEM[cert_len] = '\0';

            if(MwFim_FileWrite(MW_FIM_IDX_GP13_PROJECT_AWS_CERT_PEM, 0, MW_FIM_GP13_AWS_CERT_PEM_SIZE, (uint8_t*)&tCert) != MW_FIM_OK)
            {
                goto done;
            }
...
at_command_t g_taAppAtCmd[] =
{
    ...
    { "at+awsdevinfo",  app_at_cmd_sys_aws_dev,            "update aws device info" },
    { "at+awskey",      app_at_cmd_sys_aws_key,            "udpate aws private key" },
