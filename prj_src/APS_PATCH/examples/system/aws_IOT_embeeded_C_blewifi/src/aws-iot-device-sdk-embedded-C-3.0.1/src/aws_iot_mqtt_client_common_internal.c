...
#define PUBACK_FOR_SUBSCRIBE_QOS1_RETRY 10
...
IoT_Error_t aws_iot_mqtt_internal_send_packet(AWS_IoT_Client *pClient, size_t length, Timer *pTimer) {
    ...
    //IOT_INFO(" %s --> t expired  %d  %d", __FUNCTION__, pTimer->end_time.tv_sec, pTimer->end_time.tv_usec);
    ...
    //IOT_INFO(" %s <--q t expired  %d  %d", __FUNCTION__, pTimer->end_time.tv_sec, pTimer->end_time.tv_usec);
...
static IoT_Error_t _aws_iot_mqtt_internal_handle_publish(AWS_IoT_Client *pClient, Timer *pTimer) {
	...
	if(SUCCESS != rc) {
        for(int x = 1; x <= PUBACK_FOR_SUBSCRIBE_QOS1_RETRY; x++)
        {
            rc = aws_iot_mqtt_internal_send_packet(pClient, len, pTimer);
            if(SUCCESS == rc)
                break;
...
IoT_Error_t aws_iot_mqtt_internal_cycle_read(AWS_IoT_Client *pClient, Timer *pTimer, uint8_t *pPacketType) {
	...
	switch(*pPacketType) {
		...
		case PINGRESP: {
            ...
            IOT_INFO("PINGRESP for keepalive\n");
