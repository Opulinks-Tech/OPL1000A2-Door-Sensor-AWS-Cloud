...
#define INTERVAL_OF_POOLING_SYS_MEM (5000)
...
osTimerId    g_tAppCtrlGetSysMemTimer;
...
//    osTimerDef_t timer_sys_mem_pooling_def;
   ...
   //T_MwFim_GP12_HttpPostContent HttpPostContent;
    ...
    #if 0
    /* create sys memory monitortimer */
    timer_sys_mem_pooling_def.ptimer = osMemoryPoolPcbInfoDump;
    g_tAppCtrlGetSysMemTimer = osTimerCreate(&timer_sys_mem_pooling_def, osTimerPeriodic, NULL);
    if(g_tAppCtrlGetSysMemTimer == NULL)
    {
        BLEWIFI_ERROR("BLEWIFI: ctrl task createtimer_sys_mem_pooling timer fail \r\n");
    }
    else
    {    
        osTimerStop(g_tAppCtrlGetSysMemTimer);
        osTimerStart(g_tAppCtrlGetSysMemTimer, INTERVAL_OF_POOLING_SYS_MEM);
    }
    #endif
